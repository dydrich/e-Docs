...
class AccountManager{
	...
	public function recoveryPasswordViaEmail(){
		...
		*/
		...
		$smt = $this->datasource_->prepare("INSERT INTO rb_recupero_password (utente, token, data_richiesta, data_scadenza_token) VALUES (?, ?, ?, ?, ?)");
		$id = $this->user_->getUid();
		$dt = $due->format("Y-m-d H:i:s");
		$smt->bind_param("iisss", $id, $uniqid, $now, $dt);
		$smt->execute();
